...
from src.utils.exceptions import InsertionError, DatabaseConnectionError, DocumentValidationError, UpdateError
...
def _update(documents: Union[List[dict], dict], db_name: str, collection_name: str) -> dict:
    ...
    try:
        ...
        if result.modified_count > 0:
            return {"success": True, "message": "Successfully updated documents.", "result": result}
        else:
            raise UpdateError(f"Failed bulk update. No documents were updated.")
...
def update_records(collection: Collections, documents: Union[List[dict], dict], safe_mode: bool = True):
    ...
    try:
        ...
        logger.error(f"Failed to update records in {collection_name}: {str(e)}")
        raise UpdateError(f"Failed to update records. Info: {str(e)}")
